...
Vocaboost のアイコンを決めるために参照した、プラットフォーム公式・コミュニティで広く引用される指針の要約です。最終判断はプロダクトのトーン（TOEIC・学習・オフライン）と既存 UI（ダーク基調・ゴールドのアクセント）に合わせています。
...
2. **ウェブ（タブ・メタデータ）**: `layout.tsx` の `metadata.icons` と `manifest.webmanifest` の先頭エントリで **マスター PNG を直接参照**（現状のピクセルサイズを `sizes` に記載。差し替え時は寸法を合わせて更新すること）。
3. **`any` 用（派生）**: マスターを **192 / 512** にリサイズ（透過は維持）。`icon-192.png` / `icon-512.png`（マニフェストの追加候補・キャッシュ向け）。
4. **`maskable` 用**: **512×512 不透明の黒地**に、マスターを **約 78%** で中央合成した **icon-maskable-512.png**（Android ランチャー向け）。
5. **書き出し副産物**: **apple-touch-icon.png**（180×180・黒地合成）は `icons:export` で引き続き生成（ホーム画面でマスター直参照が不十分な場合の予備）。
