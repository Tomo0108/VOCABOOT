...
2. **`any` 用**: `icon.svg` とそこから書き出した **192 / 512 PNG**（汎用インストール・タブ用）。
3. **`maskable` 用**: 同一デザインを**中央にやや縮小**して配置した **512×512 PNG** を別ファイルにし、円形マスクでも V とカードが切れないようにする（web.dev の安全域の考え方に沿う）。
4. **Apple**: **180×180** の `apple-touch-icon.png` を用意し、Next の metadata から参照する。
