...
1. **トーン**: ダーク背景（`#0a0a0a` 系）と UI のアクセントに近い**ゴールド**で「V」を主題にし、白いカードで「単語カード／学習」を連想させる。
2. **`any` 用**: **正方形の透過キャンバス**に、**黒は角丸タイル内のみ**（外周はアルファ透過）。`icon.svg` およびそこから書き出した **192 / 512 PNG**。
3. **`maskable` 用**: ランチャーで透過が白埋めになりやすいため **512×512 全面不透明**の黒地に、同一図形を**中央で縮小**して載せた **icon-maskable-512.png**（安全域の考え方に沿う）。
4. **Apple**: iOS のホーム画面では透過が白などで埋まりがちなため、`apple-touch-icon.png`（180×180）は **maskable 系 SVG から**書き出し（不透明）。

アセットの生成手順は `package.json` の `icons:export`（`scripts/export-pwa-icons.mjs`）に集約しています。
