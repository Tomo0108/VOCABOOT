...
1. **モチーフ**: 初期デザインの**白い単語カード**＋**3本の罫線**＋右下の淡い円（装飾）。背景は**黒（`#000000`）**の角丸正方形（`rx="112"`）で全面塗り。
2. **アクセント色**: 罫線のストロークは **RGB(240, 191, 76)**（`#F0BF4C`）。
3. **`any` 用**: `icon.svg` と **192 / 512 PNG**（不透明・正方形）。
4. **`maskable` 用**: **512×512 全面黒**の上に、同一アートを**約 78% 縮小**して中央配置した **icon-maskable-512.png**。
5. **Apple**: `apple-touch-icon.png`（180×180）は **maskable 系 SVG** から書き出し。
